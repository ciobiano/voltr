...
interface ExpandSectionProps {
  ...
  headline: string;
  ctas: [ExpandSectionCta, ExpandSectionCta];
}

export function ExpandSection({ image, headline, ctas }: ExpandSectionProps) {
  ...
  useGSAP(
    () => {
      ...
      if (!section || !card || !headlineEl || !cta) return;
  ...
  return (
    <div
      ref={sectionRef}
      data-expand-section
      className="relative h-screen bg-surface-secondary overflow-hidden -mt-2 m-2"
    >
      ...
      >
        {ctas.map((cta) => (
          <a
            ...
            className={
              cta.tone === "dark"
                ? "flex flex-col justify-between gap-8 md:gap-12 p-5 md:p-6 rounded-lg bg-obsidian border border-white/10 group hover:brightness-110 transition-all"
                : "flex-1 flex flex-col justify-between gap-8 md:gap-12 p-5 md:p-6 rounded-lg bg-orange group hover:brightness-105 transition-all"
            }
          >
            <span className="text-size-3xs text-accent">{cta.eyebrow}</span>
            <div className="flex items-end justify-between">
              ...
              <div
                className={
                  cta.tone === "dark"
                    ? "h-9 w-9 rounded-full bg-white/10 flex items-center justify-center group-hover:bg-orange transition-colors shrink-0"
                    : "h-9 w-9 rounded-full bg-text-primary/20 group-hover:bg-accent flex items-center justify-center shrink-0"
                }
              >
                <ArrowRight
                  size={14}
                  className={cta.tone === "dark" ? "text-white" : "text-white group-hover:text-orange"}
                />
              </div>
            </div>
          </a>
